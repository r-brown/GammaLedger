...
class GammaLedger {
     ...
     */
    async autoFillUnderlyingPrice(inputElement: HTMLInputElement | null) {
        if (!inputElement) {
            return;
        }
    ...
    async handleAIQuickPrompt(prompt: string, options: { promptType?: string | null; [key: string]: unknown } = {}) {
    ...
    async renderTradePayoffChart(trade: Record<string, unknown>, chartId: string, footnoteId: string) {
    ...
    async getUnderlyingPriceForPayoff(trade: Record<string, unknown> = {}) {
    ...
    async importOfxContent(raw: string, context: Record<string, unknown> = {}) {
    ...
    async importRobinhoodCsvContent(raw: string, context: Record<string, unknown> = {}) {
